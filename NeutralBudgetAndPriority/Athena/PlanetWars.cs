...
namespace AthenaBot
{
    public class PlanetWars
    {
        ...
        public int Distance(int sourcePlanet, int destinationPlanet)
        {
            ...
            double dx = source.Location.X - destination.Location.X;
            double dy = source.Location.Y - destination.Location.Y;
        ...
        public static void IssueOrder(Planet source, Planet dest, int numShips)
        {
            Console.WriteLine("" + source.Id + @" " + dest.Id +
              @" " + numShips);
            Console.Out.Flush();
        ...
        public bool IsAlive(Player playerId)
        {
            foreach (Planet p in planets)
            {
                if (p.Owner == playerId)
        ...
        public int Winner()
        {
            ...
            foreach (Planet p in planets)
            {
                if (!remainingPlayers.Contains((int)p.Owner))
                {
                    remainingPlayers.Add((int)p.Owner);
                }
            }
            foreach (Fleet f in fleets)
            {
                if (!remainingPlayers.Contains((int)f.Owner))
                {
                    remainingPlayers.Add((int)f.Owner);
                }
            }
        ...
        public int NumShips(Player playerId)
        {
            int numShips = 0;
            foreach (Planet p in planets)
            {
        ...
        private void ParseGameState(string s)
        {
            ...
            for (int i = 0; i < lines.Length; ++i)
            {
                ...
                if (tokens[0].Equals("P"))
                {
                    ...
                    int owner = int.Parse(tokens[3], CultureInfo.InvariantCulture);
                    int numShips = int.Parse(tokens[4], CultureInfo.InvariantCulture);
                    int growthRate = int.Parse(tokens[5], CultureInfo.InvariantCulture);
                ...
                else if (tokens[0].Equals("F"))
                {
                    ...
                    int owner = int.Parse(tokens[1], CultureInfo.InvariantCulture);
                    int numShips = int.Parse(tokens[2], CultureInfo.InvariantCulture);
                    int source = int.Parse(tokens[3], CultureInfo.InvariantCulture);
                    int destination = int.Parse(tokens[4], CultureInfo.InvariantCulture);
                    int totalTripLength = int.Parse(tokens[5], CultureInfo.InvariantCulture);
                    int turnsRemaining = int.Parse(tokens[6], CultureInfo.InvariantCulture);
